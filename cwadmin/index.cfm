...
<cfparam name="application.cw.companyName" default="CentLogic">
<cfparam name="application.cw.adminThemeDirectory" default="default">
<cfparam name="session.cw.userAlert" default="">

<!--- CWMURA [start]: auto login from mura admin : 2011-12-22 --->
<cfif isDefined('session.mura.isloggedin') and session.mura.isloggedin is true>
	<cfset form.adminUsername = session.mura.username>
	<!--- set cw session vars --->
	<cfset session.cw.LoggedIn = 1>
	<!--- This session store the username --->
	<cfset session.cw.LoggedUser="#session.mura.username#">
	<!--- get level from mura --->
	<cfif listFind(session.mura.memberships,'DataEntry',';')>
		<cfset session.cw.AccessLevel = 'manager'>
	<cfelseif listFind(session.mura.memberships,'S2')>
		<cfset session.cw.AccessLevel = 'developer'>
	<!--- default to merchant level --->
	<cfelse>
		<cfset session.cw.AccessLevel = 'merchant'>
	</cfif>
	<!--- QUERY: set installation date here since we're bypassing usual first login --->
...
<!--- if mura not logged in, send to login page --->
<cfelse>
	<cflocation addtoken="no" url="/admin/">
</cfif>
<!--- CWMURA [end]: auto login from mura admin : 2011-12-22 --->
...
<cfif IsDefined("form.adminUsername")>
	...
	<cfif getLogOn.RecordCount>
		...
		<cfelse>
			...
			<cfif session.cw.accessLevel is 'developer' and application.cw.companyemail contains 'CentLogic'>
