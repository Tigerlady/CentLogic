...
Add To Cart submission is managed by passing the submitted form variables to cw-mod-CentLogic.cfm
(For custom use of cw-mod-CentLogic, variables can come from any scope as long as the required values are present)
==========================================================
--->
...
<cfif isDefined('form.productID') AND form.productID eq request.cwpage.productID>
	...
	<!--- pass all form variables to CentLogic tag --->
	...
	<cfmodule
	template="cwapp/mod/cw-mod-CentLogic.cfm"
	...
	<div class="CWproductThumbs">
		...
		<cfloop list="#product.image_thumbnails#" index="i">
				<cfif request.cwpage.imageZoom>
					...
					<a href="#request.cwpage.zoomDir##i#" class="CWimageZoomLink CWlink" title="#product.product_name#" rel="shadowbox[body]"><img class="CWthumb" src="#request.cwpage.thumbDir##i#" alt="#product.product_name#"></a>
				<cfelse>
	...
	<cfif product.product_on_web eq 'true' and product.product_archive neq 1
		...
		<cfif len(trim(product.productImgMain))>
				...
				<cfif request.cwpage.thumbsPos is 'above'>#request.cwpage.thumbsHtml#</cfif>
					<cfif request.cwpage.imageZoom>
						<!--- show the image with zoom link --->
						<a href="#product.productImgZoom#" id="CWproduct#product.product_id#imgLink" class="CWlink" title="#product.product_name#"<cfif request.cwpage.imagethumbs lte 1> rel="shadowbox[body]"</cfif> rel="shadowbox[body]"><img src="#product.productImgMain#" alt="#product.product_name#: click to enlarge" id="CWproductImgMain"></a>
						<div>
							<a href="#product.productImgZoom#" id="CWproduct#product.product_id#zoomLink" class="CWlink" rel="shadowbox[body]" title="#product.product_name#">Click to Enlarge</a>
						</div>
					<cfelse>
						<!--- show only the image, no link, no zoom --->
