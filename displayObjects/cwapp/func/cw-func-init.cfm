...
Description: handles initialization functions for the CentLogic application
Values are set for each page request, and for the cart application
To reset the application variables, log in to the CentLogic admin
...
<cffunction name="CWinitApplication"
	...
	<cflock type="exclusive" scope="application" timeout="10" throwontimeout="no">
		...
		<!---  if logged in as admin and url.resetapplication=[debugPassword]--->

			<!--- CWMURA [start]: change this to use mura login and appreload OR cw login and reloadapplication/pw : 2011-04-20 --->
			<cfif (not isDefined('application.cw.appSiteUrlHttp'))
				...
				isDefined('session.mura.isloggedin') and session.mura.isloggedin is true
				and isDefined('url.appreload')
				)>
			<!---
			<cfif (not isDefined('application.cw.appSiteUrlHttp'))
				OR arguments.fullReset eq true
			...
			--->
			<!--- CWMURA [end]: end edit : 2011-04-20 --->
			...
			<cfcatch>
				<p>Error: Database Unavailable<br>Please refer to the CentLogic installation instructions, and verify the database and data connection are set up correctly.</p>
				<cfabort>
			</cfcatch>
			</cftry>
			...
			<cfif not (isValid('url',application.cw.appSiteUrlHttp) and left(application.cw.appSiteUrlHttp,5) eq 'http:')>
				<cfset application.cw.appSiteUrlHttp = 'https://' & cgi.server_name>
			</cfif>
			...
			<cfelse>
				<cfset application.cw.appPageCheckoutUrl = application.cw.appSiteUrlHttps & variables.urlRoot & application.cw.appPageCheckOut>
				<cfset application.cw.appPageAccountUrl = application.cw.appSiteUrlHttps & variables.urlRoot & application.cw.appPageAccount>
                <cfset application.cw.appPageConfirmOrderUrl = application.cw.appSiteUrlHttps & variables.urlRoot & application.cw.appPageConfirmOrder>
			</cfif>
			...
			<cfif REFindNoCase('http[s]?\:\/\/',application.cw.appPageResults) eq 0>
				<cfset application.cw.appPageResultsUrl = application.cw.appSiteUrlHttps & variables.urlRoot & application.cw.appPageResults>
			<cfelse>
			...
			<cfif REFindNoCase('http[s]?\:\/\/',application.cw.appPageDetails) eq 0>
				<cfset application.cw.appPageDetailsUrl = application.cw.appSiteUrlHttps & variables.urlRoot & application.cw.appPageDetails>
			<cfelse>	
			...
			<cfif REFindNoCase('http[s]?\:\/\/',application.cw.appPageShowCart) eq 0>
				<cfset application.cw.appPageShowCartUrl = application.cw.appSiteUrlHttps & variables.urlRoot & application.cw.appPageShowCart>
			<cfelse>	
...
<cffunction name="CWinitRequest"
	...
	<!--- CWMURA [edit]: request.path doesn't show up as expected here : 2014-06-05 --->
	...
	--->
	<!--- CWMURA [end]: end edit : 2014-06-05 --->

	<!--- CWMURA [request variable for CentLogic display includes]:  : 2011-12-22 --->
	<cfset request.cwpage.cwIncludePath = application.cw.appCWContentDir>
	<!--- CWMURA [edit]: set logout var for CW based on Mura 'doaction' : 2012-05-22 --->
	<cfif isDefined('url.doaction') and url.doaction is 'logout'>
		<cfset request.cwpage.logout = 1>
	</cfif>
	<!--- CWMURA [end]: end edit : 2011-12-22 --->
	...
	<cfset request.cw.sslPages = "#application.cw.appPageCheckout#,#application.cw.appPageAccount#,#application.cw.appPageConfirmOrder#">
	...
	<!--- CWMURA [edit]: mura default path : 2022-02-08 --->
	<cfparam name="application.cw.appCWassetsDir" default="/plugins/CentLogic/displayObjects/">
	...
	<cfif request.cw.thisDirName eq request.cw.adminDirName>
		...
		<!--- CWMURA [edit]: add displayObjects to path : 2011-12-22 --->
		<cfparam name="request.cwpage.adminImgPrefix" default="../displayObjects/">
		<!--- CWMURA [end]: end edit : 2011-12-22 --->
	...
	<cfif (isDefined('url.logout') and url.logout is 1)
		...
		<!--- CWMURA [edit]: log out mura user, preserving remaining cw session vars : 2012-05-22 --->
		<cfif isDefined('session.siteID')>
			<cfset variables.temp.cw = session.cw>
			<cfset variables.temp.cwclient = session.cwclient>
				<cfset $=application.serviceFactory.getBean("muraScope").init(session.siteID)>
				<cfset $.getBean('loginManager').logout()>
			<cfset session.cw = variables.temp.cw>
			<cfset session.cwclient = variables.temp.cwclient>
			<cfset structDelete(variables,'temp')>
		</cfif>
		<!--- CWMURA [end]: end edit : 2012-05-22 --->
	...
	<cfif isArray(application.cw.pluginDirectories) and arrayLen(application.cw.pluginDirectories) gt 0>
		...
		<cfloop from="1" to="#arrayLen(application.cw.pluginDirectories)#" index="i">
			<cfset variables.pluginFileName = expandPath('../plugins/#application.cw.pluginDirectories[i]#/config.cfm')>
			<cftry>
