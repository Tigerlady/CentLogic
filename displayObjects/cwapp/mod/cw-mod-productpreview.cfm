	...
	<cfsavecontent variable="imgHtml">
		<cfoutput>
			<cfif attributes.show_image neq false and len(trim(productImg))>
				...
				<cfif len(trim(attributes.details_page))>
					<a href="#detailsUrl#&amp;#product.product_name#" class="CWlink" title="#product.product_name#">
	...
	<cfsavecontent variable="cartFormHtml">
		...
		<cfif isDefined('url.secondary') and url.secondary gt 0><cfset formActionVars = listAppend(formActionVars,'secondary')></cfif>
			...
			<cfif len(trim(product.optiontype_ids)) and attributes.sku_id eq 0>
				...
				document.write('<input type="button" value="Add to Cart&nbsp;&raquo;" class="CWaddButton CWformButton" id="CWaddButton-#product.product_id#">');
...
<a name="product-#product.product_id#"></a>
	...
	<cfif attributes.show_price neq false>
		<!--- price range --->
		<div id="CWproductPrices-#product.product_id#" class="CWproductPreviewPrice">
			<div class="CWproductPrice<cfif product.hasDiscount eq 'true'> strike</cfif>">
				Price:
	...
	<!--- product description  commented out to not show preview description 03/26/2015
	...
	<span class="CWproductDetailsLink"><a class="CWlink" href="#detailsUrl#&amp;#product.product_name#">#trim(attributes.details_link_text)#</a></span>
	</cfif>
</div>
<!--- /end CWproduct --->
</cfoutput>
